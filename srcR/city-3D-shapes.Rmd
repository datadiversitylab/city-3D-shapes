...
for (city_name in city_names_list) {
  ...
  # Save the plot as a .png file without labels or background
  ggsave(paste("../results/figures/", city_name, "-area-vs-height-map.png"), plot = p, width = 8, height = 6, units = "in", dpi = 300, bg = "transparent")
...
latex_table <- xtable(combined_table, caption = "Summary Table", label = "tab:summary_table")

# Save the LaTeX code to a file
latex_file <- "../docs/combined_summary.tex"
print(latex_table, file = latex_file)
...
latex_file <- "../docs/combined_summary.tex"
tex_content <- readLines(latex_file)

# Find the index where the table content starts and ends
start_index <- grep("\\\\begin\\{table\\}", tex_content)
end_index <- grep("\\\\end\\{table\\}", tex_content)

# Extract the table content
table_content <- tex_content[(start_index):(end_index)]

# Insert the preamble, table content, and postamble
modified_tex_content <- c(preamble, table_content, postamble)

# Write the modified content to the same .tex file
writeLines(modified_tex_content, latex_file)

# Open the .tex file in RStudio
rstudioapi::navigateToFile(latex_file)

# Wait for a few seconds to ensure the file is opened
Sys.sleep(3)

# Compile the .tex file into a PDF
rstudioapi::executeCommand("compilePDF")

# Wait for the compilation process to finish
Sys.sleep(3)

# Path to the output PDF file
pdf_file <- tools::file_path_sans_ext(latex_file)  # Remove the .tex extension
pdf_file <- paste0(pdf_file, ".pdf")

# Check if the PDF file exists
if (file.exists(pdf_file)) {
  message("PDF file successfully generated: ", pdf_file)
} else {
  warning("PDF file generation failed.")
}
